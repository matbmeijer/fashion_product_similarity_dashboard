...
## Introduction
The .R module **Fashion Product Similarity Dashboard** is a simple workflow to show an introductory starting point to identify product similarities based solely on a fashion item product image - no additional data is used. To see the full output, visit the following link:
...
3. This process does not work in the 'wild' - as for example Instagram images - for the following reasons: Multiple products in same image requires image detection & image segmentation (identify if there are any fashion items in the image, and if so, segment them).
